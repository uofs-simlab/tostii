...
namespace Adr
{
  ...
  // @sect3{Implementation of the <code>AdrEquation</code> class}
  ...
  void AdrEquation<dim>::evaluate_diffusion(
    ...
    // Reuse the already initialized direct solver for efficiency
  ...
  void AdrEquation<dim>::id_minus_tau_J_advection_inverse(
    ...
    // Initialize and factorize the modified matrices
  ...
  // Having so implemented these post-processors. 
  // As in many other time-dependent tutorial programs, we attach flags to
  ...
  void AdrEquation<dim>::run() {
    ...
    tostii::runge_kutta_method            advection_step_method{tostii::SDIRK_TWO_STAGES};
    tostii::runge_kutta_method            reaction_step_method{tostii::HEUN2};
    tostii::ImplicitRungeKutta<BlockVector<double>, time_type>  diffusion_stepper_method(diffusion_step_method);
    tostii::ImplicitRungeKutta<BlockVector<double>, time_type>  advection_stepper_method(advection_step_method);
    tostii::ExplicitRungeKutta<BlockVector<double>, time_type>  reaction_stepper_method(reaction_step_method);
