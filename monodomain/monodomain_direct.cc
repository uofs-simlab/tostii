...
void MonodomainProblem<dim>::solve_rhs_tissue(
  ...
  solver.solve(mass_matrix, yout.block(0), tmp_rhs);
...
void MonodomainProblem<dim>::solve_lhs_tissue(
  ...
  PETScWrappers::SparseDirectMUMPS solver(solver_control, mpi_communicator);
  solver.set_symmetric_mode(true);
  solver.solve(system_matrix, yout.block(0), tmp_rhs);
  // TODO: Add options for logging solve stats
...
void MonodomainProblem<dim>::os_time_stepping(
  ...
  tostii::ExplicitRungeKutta<LA::MPI::BlockVector> tissue_ts(tissue_method);
  // ImplicitRungeKutta<LA::MPI::BlockVector> tissue_ts(tissue_method);
...
template <int dim> void MonodomainProblem<dim>::time_integrate() {
  ...
  os_t os_ts_method = tostii::os_method.at(parameters.os_time_stepping_method);
