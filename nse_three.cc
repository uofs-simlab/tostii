...
namespace StepOS
{
  ...
  private:
    ...
    void evaluate_spatial_rhs(const time_type,    //
                              const vector_type&, //
                              vector_type&);
    void id_minus_tau_J_inverse(const time_type,    //
                                const time_type,    //
                                const vector_type&, //
                                vector_type&);
    void do_phase_step(time_type,          //
                       time_type,          //
                       const vector_type&, //
                       vector_type&);
    // void do_diffusion_step(time_type, //
    //                        time_type, //
    //                        vector_type&);
    void do_reaction_step(time_type,          //
                          time_type,          //
                          const vector_type&, //
                          vector_type&);
  ...
  void NonlinearSchroedingerEquation<dim>::assemble_matrices()
  {
    ...
    FullMatrix<value_type> cell_matrix_jacobian(dofs_per_cell, dofs_per_cell);
  ...
  template <int dim>
  void NonlinearSchroedingerEquation<dim>::do_phase_step(
      time_type /*t*/,                 //
      time_type             step_size, //
      const vector_type& yin,       //
      vector_type&       yout) {
  ...
  template <int dim>
  void NonlinearSchroedingerEquation<dim>::do_reaction_step(
      time_type /*t*/,              //
      time_type          step_size, //
      const vector_type& yin,       //
      vector_type&       yout) {

    Vector<double> potential(dof_handler.n_dofs());

    // --------------------------------------------------------------------------
    //     Pack the potential vector
    //     ------------------------
    ...
    const value_type i = {0, 1};
    for (unsigned int ind=0; ind< yin.size(); ++ind) {
      yout[ind] = std::exp(-i * potential[ind] * step_size) * yin[ind];
    }
  }
  ...
  void NonlinearSchroedingerEquation<dim>::run()
  {
    ...
    tostii::Exact<vector_type, time_type> phase_method;
    tostii::Exact<vector_type, time_type> reaction_method;
    tostii::runge_kutta_method            diffusion_rk_method{tostii::CRANK_NICOLSON};
    tostii::ImplicitRungeKutta<vector_type, time_type> diffusion_method(
        diffusion_rk_method, //
        1);               // single iteration (linear system)

    /* Define OSoperators to use in the operator split stepper */
    tostii::OSoperator<vector_type, time_type> phase_stepper{
        &phase_method,
        ...
        [this](const time_type    t,   //
               ...
               const vector_type& yin, //
               vector_type&       yout) {
          this->do_phase_step(t, dt, yin, yout);
        }};

    tostii::OSoperator<vector_type, time_type> reaction_stepper{
        &phase_method,
        [this](const time_type /*t*/,  //
               const vector_type& yin, //
               vector_type&       yout) { yout = yin; },
        [this](const time_type    t,   //
               ...
               const vector_type& yin, //
               vector_type&       yout) {
          this->do_reaction_step(t, dt, yin, yout);
        }};

    tostii::OSoperator<vector_type, time_type> diffusion_stepper{
        &diffusion_method,
        [this](const time_type    t,   //
               const vector_type& yin, //
    ...
    // std::string os_type{"Milne_2_2_c_i"};
    // OperatorSplitSingle<vector_type, time_type> os_stepper(
    //     solution,                                                      //
    //     std::vector<OSoperator<vector_type, time_type>>{half_stepper,  //
    //                                                     full_stepper}, //
    //     os_complex[os_type]);

    // std::string os_name{"PP_3_A_3_c"};
    // auto        os_coeffs = tostii::os_complex.at(os_name);

    std::string os_name{"Godunov3"};
    auto        os_coeffs = tostii::os_method.at(os_name);

    tostii::OperatorSplitSingle<vector_type, time_type> os_stepper(
        solution,                                                           //
        std::vector<tostii::OSoperator<vector_type, time_type>>{phase_stepper,      //
                                                        reaction_stepper,   //
                                                        diffusion_stepper}, //
        os_coeffs);

    // Step 0 output:
    auto full_step_name{RK_method_enum_to_string(diffusion_rk_method)};
    output_results(os_name+":Exact,Exact,"+full_step_name);
